      ...
      <Box className="FooterMain" sx={{ display: 'flex', textAlign: { xs: 'center', md: 'left' }, mb: 6 }}>
        <Box className="footerMenu" sx={{ flexDirection: 'column', justifyContent: 'center', flex: 1, textAlign: 'center',}}>
          ...
          <Box key={`fm-${item.id}`} sx={{  mb: 5,}}>
            <Link sx={{fontFamily:"CoinbaseTextMedium",fontSize: 20,color: '#FFF',textDecoration: "none",cursor:"pointer","&:hover": {
              fontWeight:"bold",
          },}}>{item.title}</Link>
          </Box>
          ))}
          ...
          {SOCIALS.map(item => (
            <Box key={`si-${item.id}`} sx={{ display: 'flex',flexDirection:{xs:"column",sm:"row", md:"row"}, alignItems: 'center', mb: 3 }}>
              ...
              <Box>
                ...
                <Typography sx={{fontFamily:"CoinbaseTextMedium", fontSize: 14 }}>{item.content}</Typography>
              </Box>
            </Box>
          ))}
        </Box>
